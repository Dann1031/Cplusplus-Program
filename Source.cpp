...
tuple<int, int, int, int> timecheck() {
	//created tuple to manage edit manage and track time in one function in order, seconds to hours
	if (second >= 60) {
		second = 0;
		minute++;
	}
	if (minute >= 60) {
		...
		milHour++;
		minute = 0;
		// checks if minutes are more than 60.
	}
...
void printTime() {   
	...
	cout << "*       ";    show12HourTime();
	cout << "    *" << "  *        ";    showHourTime();    
	cout << "         *" << endl;    starPrint(27, 2);
	cout << endl;    this_thread::sleep_for(chrono::seconds(1));
